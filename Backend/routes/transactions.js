  ...
  .get(authenticateToken, async (req, res) => {
    try {
      ...
      const currentCashBalance =
        portfolioInfo.length > 0 ? portfolioInfo[0].cash_balance : 0;
      ...
      const formattedTransactions = transactions.map((transaction) => {
        ...
        const transactionImpact = isNegative
        ...
        return {
          ...
          formatted_price: `$${transaction.price_per_share.toLocaleString(
            "en-US",
      ...
      const buyOrders = transactions.filter(
        (t) => t.transaction_type === "BUY"
      ).length;
      const sellOrders = transactions.filter(
        (t) => t.transaction_type === "SELL"
      ).length;

      const summary = {
        totalTransactions,
        netInvestment: `$${totalInvestment.toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}`,
        buyOrders,
    ...
    try {
      ...
      if (!symbol || !transaction_type || !shares || !price_per_share) {
        return res.status(400).json({
          ...
          message:
            "Symbol, transaction type, shares, and price per share are required",
        });
      }
